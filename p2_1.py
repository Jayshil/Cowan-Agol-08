...
# Planetary parameters Gaudi et al. (2017) unless otherwise noted
per, per_err = 1.4811235, 0.0000011
tc, tc_err = 2457095.68572, 0.00014
bb, bb_err = 0.177, 0.014
inc, inc_err = 86.79, 0.25
rprs, rprs_err = 0.08228, 0.00043
rho, rho_err = 0.2702*1e3, 0.0029*1e3
rst = 2.362
ar, ar_err = 3.153, 0.011
...
tim, fl, fle = {}, {}, {}
tim7, fl7, fle7 = np.loadtxt(os.getcwd() + '/Analysis/TESS14.dat', usecols=(0,1,2), unpack=True)
...
hyper_pc = [[0.e-6, 1000.e-6], [-1., 1.], [-1., 1.], 0., 0.]
...
## GP priors
par_gp = ['GP_S0', 'GP_Q', 'GP_rho']
dist_gp = ['uniform', 'uniform', 'uniform']
hyper_gp = [[np.exp(-30.), np.exp(-20.)], [np.exp(-10.), np.exp(5.)], [0.1, 10.]]
...
def loglike(x):
    ...
    _, _, _, _, log_like = \
        evaluate_CowanPC(times=tim[instrument], fluxes=fl[instrument], errors=fle[instrument], t0=parameters['t0_p1'],\
                         ...
                         mflx=parameters['mflux_' + instrument], sigw=parameters['sigma_w_' + instrument], GP=True, GP_S0=parameters['GP_S0'],\
                         GP_Q=parameters['GP_Q'], GP_rho=parameters['GP_rho'], LTTD=True, rst=rst)
...
_, _, _, total_model, _ = \
    evaluate_CowanPC(times=tim[instrument], fluxes=fl[instrument], errors=fle[instrument], t0=np.nanmedian(samples['t0_p1']),\
                     ...
                     GP=True, GP_S0=np.nanmedian(samples['GP_S0']), GP_Q=np.nanmedian(samples['GP_Q']), GP_rho=np.nanmedian(samples['GP_rho']),\
                     LTTD=True, rst=rst)
